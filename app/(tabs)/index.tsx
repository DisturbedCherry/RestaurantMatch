...
export default function CentralPartScreen() {
  ...
  const openWebsite = async (websiteUrl: string, restaurantName: string) => {
    try {
      ...
      if (supported) {
        ...
        Alert.alert(
          "Cannot Open Website",
          `Nie można otworzyć strony dla ${restaurantName}. URL: ${url}`,
          [{ text: "OK" }]
        );
      }
    } catch (error) {
      ...
      Alert.alert(
        "Error",
        `Nie udało się otworzyć strony dla ${restaurantName}`,
        [{ text: "OK" }]
      );
    }
  };
  ...
  const handleFindRestaurant = () => {
    // Add navigation or action for finding restaurants
    router.push('/three');
  };
      ...
      onPress={() => {
        if (item.website) {
          ...
          Alert.alert(
            ...
            `${item.nameOfRestaurant} nie ma dodanej strony internetowej.`,
            [{ text: "OK" }]
          );
        }
      }}
          ...
          <FlatList
            ...
            ListFooterComponent={
              <TouchableOpacity 
                ...
                activeOpacity={0.8}
              >
                <Text style={styles.findRestaurantButtonText}>
                  Chcę znaleźć restaurację
                </Text>
              </TouchableOpacity>
            }
          />
...
const styles = StyleSheet.create({
  ...
  planContainer: {
    backgroundColor: 'rgba(142, 90, 255, 0.2)',
    paddingHorizontal: 8,
    paddingVertical: 4,
    borderRadius: 6,
